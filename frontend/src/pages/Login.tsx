  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        setError(response.error || 'Login failed. Please check your email.');
      }
    } catch (err: any) {
      console.error('Login error:', err);
      // Extract error message from axios response
      const errorMessage = err.response?.data?.error || err.response?.data?.message || err.message || 'Failed to login. Please try again.';
      ...
      <div className="max-w-md w-full">
        ...
        <div className="mt-6 text-center">
          <p className="text-sm text-gray-500">
            For demo purposes, use any email to login or register
          </p>
        </div>
      </div>
    </div>
